...
```bash
# Public hostname for Caddy automatic HTTPS
HONCHO_DOMAIN=honcho.example.com

# Optional API keys (required for LLM features)
OPENAI_API_KEY=your-openai-api-key
ANTHROPIC_API_KEY=your-anthropic-api-key

# External PostgreSQL database with pgvector support
...
```bash
curl https://honcho.example.com/health
```
