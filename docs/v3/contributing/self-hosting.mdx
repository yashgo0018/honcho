...
Edit `.env` and configure your LLM provider, external `DB_CONNECTION_URI`, and `HONCHO_DOMAIN` — see [LLM Setup](#llm-setup) above. For automatic HTTPS, point `HONCHO_DOMAIN` DNS at this host and allow inbound ports 80 and 443. Auth is disabled by default (`AUTH_USE_AUTH=false`).
...
This starts four services: **caddy** (ports 80/443, automatic HTTPS), **api** (port 8000 bound to `127.0.0.1` for local debugging), **deriver** (background worker), and **redis** (port 6379 bound to `127.0.0.1`). Redis caching is enabled by default. Postgres is expected to be an external database configured via `DB_CONNECTION_URI`.
...
# Health check (confirms the process is up; use https://$HONCHO_DOMAIN/health in production)
...
```bash
DB_CONNECTION_URI=postgresql+psycopg://user:password@host:5432/database
AUTH_USE_AUTH=false
LOG_LEVEL=DEBUG
```
...
The default compose file is production-oriented: Caddy terminates HTTPS on ports 80/443, the API and Redis are only exposed directly on `127.0.0.1`, restart policies are enabled, and Redis caching is enabled.

### Security
- Set `AUTH_USE_AUTH=true` and generate a JWT secret with `python scripts/generate_jwt_secret.py`
- The Docker setup includes Caddy for automatic HTTPS. Set `HONCHO_DOMAIN` in `.env`, point DNS at the host, and allow inbound ports 80 and 443.
- For manual/non-Compose deployments, put a reverse proxy in front of Honcho. Example with nginx:
  ```nginx
  server {
      listen 443 ssl;
...
### Backups
- Set up regular PostgreSQL backups with your database provider. If your external database allows direct `pg_dump` access:
  ```bash
  # One-off backup
